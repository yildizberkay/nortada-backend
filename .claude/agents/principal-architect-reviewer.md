...
- `docs/architecture.md` — Splash conventions + deltas from the reference.
...
3. **DI (Splash delta)** — domain-module wiring (`create<Domain>Module`), repos internal, services returned, cross-domain deps explicit in `buildContainer`. Flag any reintroduction of a central mega-factory or hidden global getters. Constructors must not do heavy work / touch config-db at build.
